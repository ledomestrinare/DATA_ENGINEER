...
class MercadoBitcoinApi(ABC):
    ...
    @on_exception(expo, ratelimit.exception.RareLimitException, max_tries=10)
    @ratelimit.limits(calls=29, period=30)
    @on_exception(expo, ratelimit.exception.HTTPError, max_tries=10)
...
class DaySummerApi(MercadoBitcoinApi):
    type = "day-summary"
...
class DataWriter:
    def __init__(self, coin: str, api: str) -> None:
        self.coin = coin
        self.api = api
        timenow = datetime.datetime.now().strftime("%d-%m-%Y--%H-%M-%S-%f")
        self.filename = f"{self.api}/{self.coin}/{timenow}.json"
       
    def _writer_row(self, row: str) -> None:
    ...
    def write(self, data: [dict, List]):
        if isinstance(data, dict):
            self._writer_row(json.dumps(data) + "\n")
